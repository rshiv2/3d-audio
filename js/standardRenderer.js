...
var StandardRenderer = function ( webglRenderer, teapots, dispParams ) {
	...
	const musicNode = listener.context.createBufferSource();
	const splitterNode = listener.context.createChannelSplitter(2);
	const lNode = listener.context.createConvolver();
	const rNode = listener.context.createConvolver();
	const mergerNode = listener.context.createChannelMerger(2);
	
	// Connect the audio nodes.
	musicNode.connect(splitterNode);
	splitterNode.connect(lNode, 0);
	splitterNode.connect(rNode, 1);
	lNode.connect(mergerNode, 0, 0);
	rNode.connect(mergerNode, 0, 1);
	mergerNode.connect(listener.context.destination);
	
	// Load the audio into the music node.
	...
	window.onclick = () => {
		if (listener.context.state === "suspended") {
			...
			listener.context.suspend().then(console.log("Audio context suspended!"));
		}
	}
